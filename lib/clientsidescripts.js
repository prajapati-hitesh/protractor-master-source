 ...
 * Find buttons by textual content.
 *
 * @param {string} protractorName The exact text to match.
 * @param {Element} using The scope of the search.
 *
 * @return {Array.<Element>} The matching elements.
 */
functions.findByProtractorName = function(protractorName, using) {

  using = using || document;

  var elements = using.querySelectorAll('[protractor-name]');
  var matches = [];
  for(var i=0; i<element.length; ++i) {
    var element = elements[i];
    if(element.getAttribute('protractor-name').trim() === protractorName.trim()) {
      matches.push(element);
    }
  }
  return matches;
